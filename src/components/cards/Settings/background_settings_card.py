...
class BackgroundImageCard(SettingCard):
    ...
    def __init__(self, config_manager, lang=None, parent=None):
        ...
        super().__init__(
            FluentIcon.PHOTO,
            self._get_text("background_image_title"),
            self._get_text("background_image_description"),
            parent
        )
    ...
    def _setup_ui(self):
        """设置UI界面"""
        # 选择图片按钮
        self.selectButton = PushButton(self._get_text("select_image"))
        self.selectButton.setFixedWidth(120)
        
        # 清除图片按钮
        self.clearButton = PushButton(self._get_text("clear_image"))
    ...
    def _select_image(self):
        ...
        try:
            ...
            file_dialog.setNameFilter(
                self._get_text("image_filter") + " (*.png *.jpg *.jpeg *.bmp *.gif *.webp)"
            )
            ...
            if file_dialog.exec_() == QFileDialog.Accepted:
                ...
                if selected_files:
                    ...
                    # 显示成功提示
                    InfoBar.success(
                        title=self._get_text("success"),
                        content=self._get_text("image_selected"),
            ...
            InfoBar.error(
                title=self._get_text("error"),
                content=self._get_text("image_select_failed"),
                orient=Qt.Horizontal,
                isClosable=True,
                position=InfoBarPosition.TOP,
                duration=3000,
    ...
    def _clear_image(self):
        ...
        try:
            ...
            InfoBar.success(
                title=self._get_text("success"),
                content=self._get_text("image_cleared"),
                orient=Qt.Horizontal,
                isClosable=True,
                position=InfoBarPosition.TOP,
                duration=2000,
...
class BackgroundOpacityCard(SettingCard):
    ...
    def __init__(self, config_manager, lang=None, parent=None):
        ...
        super().__init__(
            ...
            self._get_text("background_opacity_title"),
            self._get_text("background_opacity_description"),
            parent
        )
...
class BackgroundBlurCard(SettingCard):
    ...
    def __init__(self, config_manager, lang=None, parent=None):
        ...
        super().__init__(
            FluentIcon.IOT,
            self._get_text("background_blur_title"),
            self._get_text("background_blur_description"),
            parent
        )
    ...
    def _update_value_label(self, value):
        """更新数值标签"""
        if value == 0:
            self.valueLabel.setText(self._get_text("blur_disabled"))
        else:
