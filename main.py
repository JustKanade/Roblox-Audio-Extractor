...
# 导入自定义主题颜色卡片
try:
    from components.cards.custom_theme_color_card import CustomThemeColorCard
except ImportError:
    try:
        # 尝试从旧路径导入（向后兼容）
...
# 导入版本检测卡片
try:
    from components.cards.version_check_card import VersionCheckCard
except ImportError:
    try:
        # 尝试从旧路径导入（向后兼容）
...
# 导入日志控制卡片
try:
    from components.cards.log_control_card import LogControlCard
except ImportError:
    try:
        # 尝试从旧路径导入（向后兼容）
...
• {lang.get('history_file_size', f"{file_size:.1f}")}
    ...
    def setupSettingsInterface(self):
        ...
        if CustomThemeColorCard is not None:
            try:
                import components.cards.custom_theme_color_card as custom_theme_color_card
            except ImportError:
                try:
        ...
        if VersionCheckCard is not None:
            try:
                import components.cards.version_check_card as version_check_card
            except ImportError:
                try:
